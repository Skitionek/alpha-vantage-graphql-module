...
expect.extend({
  toBeType(received, argument) {
    if (argument === String && typeof received === "string") return okObject;
    if (argument === Number && typeof received === "number") return okObject;
  ...
  toMatchGraphQLList(received, node, options) {
    if (received !== null)
      expect(received).toEqual(
        expect.arrayContaining([expect.toMatchSchema(node.ofType, options)]),
      );
    return okObject;
  },
  toMatchGraphQLObjectType(received, node, options = {}) {
    ...
    if (fieldsOfInterest)
      schemaFields = schemaFields.filter(
        ([k]) => fieldsOfInterest.indexOf(k) !== -1,
      );
  ...
  toMatchGraphQLNonNull(received, node, options = {}) {
    if (received === null)
      throw Error(`Found null in field of type ${node.inspect()}`);
  ...
  toMatchSchema(received, node, { depth, path = [], customFields = [] } = {}) {
    ...
    if (!(--depth < 0)) {
      ...
      if (node instanceof GraphQLSchema) e.toMatchGraphQLSchema(node, options);
        ...
        e.toMatchGraphQLObjectType(node, options);
      else if (node instanceof GraphQLNonNull)
        e.toMatchGraphQLNonNull(node, options);
      else if (node instanceof GraphQLScalarType)
        e.toMatchGraphQLScalarType(node, options);
      else if (node instanceof GraphQLInterfaceType)
        e.toMatchGraphQLInterfaceType(node, options);
