...
const fieldResolver = (call, alias, field) => (p, a, c, i) => {
  ...
  if (!(p instanceof Object))
    throw new Error(
      "Fragment resolver can be only called within associated parent object.",
    );
...
export const fragmentResolver = (
  call,
  alias = call.name !== "" ? call.name : call.toString(),
  fields,
) => {
...
export function required(argumentsObj) {
  ...
  argumentMap.forEach(([key, value]) => {
    if (!value)
      throw new Error(
        `Argument '${key}' is required on either query or parent but not provided.`,
      );
  });
